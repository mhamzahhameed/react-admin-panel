...
const PaymentRequests = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      payments = payments?.filter(payment => payment.status !== "paid" && payment.status !== "invalid")
      ...
      for (const element of sehrShops) {
        ...
        const obj3 = payments.find((item) => item.business.id === obj2.id);
        if (obj2) {
          obj2["isLocked"] = obj1.isLocked
          obj2["reward"] = obj1.reward.title
          obj2['amount'] = obj3?.amount ? obj3.amount : 'no payments yet'
        }
      }
      ...
      sehrShops = sehrShops.filter(payment => payment.amount !== "no payments yet")
