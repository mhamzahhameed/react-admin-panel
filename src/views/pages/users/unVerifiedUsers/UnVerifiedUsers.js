import { cilCheckCircle, cilGroup, cilLockLocked, cilTrash, cilViewColumn } from '@coreui/icons'
...
const UnverifiedUsers = () => {
    ...
    const handleVerify = (item) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                await AxiosInstance.put(`/api/user/verify-user/{id}?id=${item.id}`)
                await fetchData()
            }
        });
    }
    ...
    const renderData = () => {
        ...
        return currentPageData.map((item, index) => (
            ...
            <CIcon icon={cilPenAlt} size="sm" /> Update
                        ...
                        <button className="btn btn-warning  text-light" onClick={() => handleLimit(item)}>
                            <CIcon icon={cilLockLocked} size="sm" /> Limit
                        </button>
                        <button className="btn btn-warning me-2 text-light" onClick={() => handleDelete(item)}>
    ...
    return (
        loader ? <Loader /> : <div className="container">
          ...
          <CCard className="mb-4 bg-info">
            ...
            <div className="card">
                <div className="card-header">UnverifiedUsers</div>
